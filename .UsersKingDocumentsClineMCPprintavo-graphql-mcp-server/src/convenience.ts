  ...
  try {
    ...
    if (order.lineItems && order.lineItems.length > 0) {
      lineItems = await Promise.all(
        order.lineItems.map((item: any) => tools.getLineItem(item.id))
      );
    }
    ...
    if (order.transactions && order.transactions.length > 0) {
      transactions = await Promise.all(
        order.transactions.map((transaction: any) => tools.getTransaction(transaction.id))
      );
    }
  ...
  try {
    ...
    if (lineItems && lineItems.length > 0) {
      ...
      const createdLineItems = await Promise.all(
        lineItems.map((item) => tools.lineItemCreate(lineItemGroupId, item))
      );
 ...
 */
export const searchCustomerDetail = async (query: string, limit: number = 5) => {
  try {
    ...
    const matchedCustomers = customers.filter((customer: any) => {
      const matchCompany = customer.companyName && customer.companyName.toLowerCase().includes(lowerQuery);
      const matchContact = customer.primaryContact && (
        (customer.primaryContact.fullName && customer.primaryContact.fullName.toLowerCase().includes(lowerQuery)) ||
    ...
    const detailedCustomers = await Promise.all(
      matchedCustomers.map(async (customer: any) => {
        ...
        const customerContacts = contacts.filter((contact: any) => 
          contact.customer && contact.customer.id === customer.id
        );
        ...
        // Calculate metrics
        const totalSpent = customerOrders.reduce((sum: number, order: any) => sum + parseFloat(order.total || 0), 0);
        ...
        return {
          ...
          recentOrders: customerOrders.slice(0, 3), // Just get 3 most recent
          metrics: {
            totalSpent,
            averageOrderValue,
            daysSinceFirstOrder,
 ...
 */
export const convertQuoteToInvoice = async (quoteId: string, statusId?: string) => {
  try {
 ...
 */
export const createCustomerWithDetails = async (
  customerData: any,
  contactData: any,
  addressData?: any
) => {
...
export const getOrderAnalytics = async (days: number = 30) => {
  try {
    ...
    const ordersByStatus = statuses.reduce((acc: any, status: any) => {
      acc[status.name] = {
        count: 0,
        value: 0,
        orders: []
    ...
    const lastWeekOrders = recentOrders.filter((order: any) => {
      const orderDate = new Date(order.createdAt);
      return orderDate >= lastWeekCutoff;
    });
    
    const lastMonthOrders = recentOrders.filter((order: any) => {
      const orderDate = new Date(order.createdAt);
      return orderDate >= lastMonthCutoff;
    });
    
    const lastWeekValue = lastWeekOrders.reduce((sum: number, order: any) => sum + parseFloat(order.total || 0), 0);
    const lastMonthValue = lastMonthOrders.reduce((sum: number, order: any) => sum + parseFloat(order.total || 0), 0);
...
export const getCustomerAnalytics = async () => {
  try {
    ...
    const customerAnalytics = await Promise.all(
      customers.map(async (customer: any) => {
        ...
        if (daysSinceLastOrder !== null) {
          ...
          else if (daysSinceLastOrder <= 90) recency = 'Medium';
          else recency = 'Old';
        }
        
        // Determine frequency
        let frequency = 'Unknown';
        if (orderCount === 0) frequency = 'None';
        else if (orderCount === 1) frequency = 'One-time';
        else if (orderFrequency && orderFrequency <= 60) frequency = 'High';
        else if (orderFrequency && orderFrequency <= 120) frequency = 'Medium';
        else frequency = 'Low';
        
        // Determine monetary value
        let monetary = 'Unknown';
        if (totalSpent === 0) monetary = 'None';
        else if (totalSpent < 500) monetary = 'Low';
        else if (totalSpent < 2000) monetary = 'Medium';
        else monetary = 'High';
    ...
    customerAnalytics.forEach((data: any) => {
    ...
    Object.keys(segmentGroups).forEach(segment => {
      ...
      const totalRevenue = customers.reduce((sum: number, data: any) => sum + data.financial.totalSpent, 0);
      
      segmentMetrics[segment] = {
        customerCount: count,
        percentOfCustomers: (count / customerAnalytics.length) * 100,
        totalRevenue,
        percentOfRevenue: totalRevenue / customerAnalytics.reduce((sum: number, data: any) => sum + data.financial.totalSpent, 0) * 100,
        averageOrderValue: customers.reduce((sum: number, data: any) => sum + data.financial.averageOrderValue, 0) / count
      };
    });
 ...
 */
export const processPayment = async (orderId: string, amount: number, paymentDetails: any = {}) => {
  try {
 ...
 */
export const getProductAnalytics = async (productQuery: string, limit: number = 10) => {
  try {
    ...
    const productAnalytics = await Promise.all(
      products.slice(0, limit).map(async (product: any) => {
...
async function callSanMarMCP(toolName: string, params: any): Promise<any> {
 ...
 */
export const createQuoteWithSanMarLiveData = async (
  customerId: string, 
  contactId: string, 
  sanmarItems: Array<{
    styleNumber: string;
 ...
 */
export const updateLineItemSizes = async (
  lineItemId: string,
  sizes: Record<string, number>
) => {
  try {
